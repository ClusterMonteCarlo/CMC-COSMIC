...
#define CHECK_PARSED(A,DEFAULT,DOC) \
	...
	// For any stars with ZAMS masses above PTS1_HIGHMASS_CUTOFF, decrease PTS1 by 10x
		...
		// rtmsflag = 0 -> uses the sse rtms for M < 200M_sun and extrapolation for z < 0.0008 and M>200m_sun.
        // rtmsflag = 1 -> interpolates the rtms from Boost tracks (Szécsi et al. (2022)). Extrapolation is used after M > 575 M_sun.
        // rtmsflag = 2 -> uses the best-fit power law for rtms vs stellar mass from BPASSv2.2 tracks (Stanway & Eldridge (2018)).
		//                 rtms data for BPASS tracks is available only till 300 m_sun and beyond that we follow the fitted power law profile.
        // default=0
