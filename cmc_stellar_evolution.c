...
void do_stellar_evolution(gsl_rng *rng){
  ...
  /* Meagan - Track mass loss from stellar evolution for innermost stars */
  M_beforeSE= 0.0;
  M10_beforeSE=0.0;
  M100_beforeSE=0.0;
  M1000_beforeSE=0.0;
  ...
  for(j=1; j<=jj+1; j++){ 
	M_beforeSE+=star[j].m;
	if (j==10) {
		M10_beforeSE = M_beforeSE;
		r10_beforeSE = star[j].r;
	}
	if (j==100) {
		M100_beforeSE = M_beforeSE;
		r100_beforeSE = star[j].r;
	}
	if (j==1000) {
		M1000_beforeSE = M_beforeSE;
	  	r1000_beforeSE = star[j].r;
	}
	if (j==N_core) {
		Mcore_beforeSE = M_beforeSE;
	  	rcore_beforeSE = star[j].r;
	}
  }	
...
//m_init, m_bh, time, id, kick, r, vr_init, vt_init, vr_final, vt_final, binflag, m0_init, m1_init, m0_final, m1_final, 
  ...
  for(j=1; j<=jj+1; j++){ 
	M_afterSE+=star[j].m;
	if (j==10) {
		M10_afterSE = M_afterSE;
	}
	if (j==100) {
		M100_afterSE = M_afterSE;
	}
	if (j==1000) {
		M1000_afterSE = M_afterSE;
	}
	if (j==N_core) { 
		Mcore_afterSE = M_afterSE;
	}
  }	

  
  dM_dt_SE10=(M10_afterSE - M10_beforeSE) / Dt; 
  dM_dt_SE100=(M100_afterSE - M100_beforeSE) / Dt; 
  dM_dt_SE1000=(M1000_afterSE - M1000_beforeSE) / Dt;
  dM_dt_SEcore=(Mcore_afterSE - Mcore_beforeSE) / Dt; 

  dprintf("central Mass loss: Time=%.18g Dt=%.18g r10_before=%.18g M10_before=%.18g M10_after=%.18g r100_before=%.18g M100_before=%.18g M100_after=%.18g r1000_before=%.18g M1000_before=%.18g M1000_after=%.18g rcore_before=%.18g Mcore_before=%.18g Mcore_after=%.18g N_core=%d\n", TotalTime, Dt, r10_beforeSE, M10_beforeSE, M10_afterSE, r100_beforeSE, M100_beforeSE, M100_afterSE, r1000_beforeSE, M1000_beforeSE, M1000_afterSE, rcore_beforeSE, Mcore_beforeSE, Mcore_afterSE, N_core); 

  dprintf("central Mass loss: Time=%.18g Dt=%.18g (dM/dt_10)/r10=%.18g (dM/dt_100)/r100=%.18g (dM/dt_1000)/r1000=%.18g (dM/dt_core)/rcore=%.18g\n", TotalTime, Dt, dM_dt_SE10/r10_beforeSE, dM_dt_SE100/r100_beforeSE, dM_dt_SE1000/r1000_beforeSE, dM_dt_SEcore/rcore_beforeSE);
}
...
void handle_bse_outcome(long k, long kb, double *vs, double tphysf)
{
  ...
  if (binary[kb].bse_mass[0] != 0.0 && binary[kb].bse_mass[1] != 0.0 && binary[kb].bse_tb > 0.0) {
    ...
    /* Two kicks */
       ...
       vt_add_kick(&(star[k].vt),vs[1],vs[2]);
       vt_add_kick(&(star[k].vt),vs[5],vs[6]);//will mean a different angle for each kick, should be ok as this is a randomised process...
    ...
    if (vs[4]>0.0 && vs[8]<=0.0 ) {
      ...
      /* Star knew went SN */
	    ...
	    VKO = sqrt(vs[1]*vs[1]+vs[2]*vs[2]+vs[3]*vs[3]);
            ...
            vt_add_kick(&(star[knewp].vt),vs[5],vs[6]);
       ...
       /* Star knewp went SN */
	    ...
	    VKO = sqrt(vs[1]*vs[1]+vs[2]*vs[2]+vs[3]*vs[3]);
            ...
            vt_add_kick(&(star[knew].vt),vs[5],vs[6]);
      ...
      /* Two SNe and the 2nd one disrupts the system. 
        ...
        if (vs[0]==1) {
            ...
            vt_add_kick(&(star[knew].vt),vs[9],vs[10]);
	    ...
	    VKO = sqrt(vs[1]*vs[1]+vs[2]*vs[2]+vs[3]*vs[3]+vs[9]*vs[9]+vs[10]*vs[10]+vs[11]*vs[11]);
            ...
            vt_add_kick(&(star[knewp].vt),vs[5],vs[6]);//finalise companion vt...
            ...
            vt_add_kick(&(star[knewp].vt),vs[9],vs[10]);
