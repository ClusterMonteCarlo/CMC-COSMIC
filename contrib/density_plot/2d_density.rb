...
if (ARGV.length < 4 || ARGV.length > 5)
  print "Usage: ./2d_density.rb <file_prefix> <snapshot #> <half mass radius> <unit of length> <'ns' if to ignore 1.4 Msun stars \n e.g. ./2d_density.rb w6_n1e5_fb0.03.out 0076 0.804557 3.80157 ns\n"
  exit
end
...
class D_compute
  attr_accessor :mass, :lum, :rad, :rho, :rhoL, :rRad_m, :rRad_M, :rRad_ratio, :unit_l
  def initialize(mass = [], lum = [], rad = [], rho = [], rhoL = [], rRad_m = 0, rRad_M = 0, rRad_ratio = 0, unit_l = 0)
         @mass = mass
         @lum = lum
         @rad = rad
         @rho = rho
...
gz.each_line { |line|
  if line =~ /\A[0-9]+/
    ...
    # Neutron flag on?
    if ns_flag == "ns"
      # Four possibilities for neutron stars
      # Alone
      if (info_array[7].to_i == 0 && info_array[1].to_f >= 1.4)
        compute.lum.push(0.0)
        # In binary with normal star
      elsif (info_array[7].to_i == 1 && info_array[8].to_f >= 1.4 && info_array[9].to_f < 1.4)
        compute.lum.push(info_array[9].to_f**3)
        # In binary with normal star
      elsif (info_array[7].to_i == 1 && info_array[9].to_f >= 1.4 && info_array[8].to_f < 1.4)
        compute.lum.push(info_array[8].to_f**3)
        # In binary with other neutron star
      elsif (info_array[7].to_i == 1 & info_array[8].to_f >= 1.4 && info_array[9].to_f >= 1.4)
        compute.lum.push(0.0)
        # If not a neutron star system... check if a binary
      else
